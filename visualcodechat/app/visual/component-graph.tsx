...
const origindata = {
  ...
  const ComponentsGraph: React.FC<Props> = ({ json, messageId}) => {
...
function boundingForce(width: number, height: number) {
      ...
      function isHighlighted(node: any) {
        if(forcedata.currentNodes){
          return forcedata.currentNodes.some((link: { source: any; target: any; }) => link.source === node.id || link.target === node.id);
        }
      }

      function isHighlightedLink(link: any) {
        if(forcedata.currentNodes){
          return forcedata.currentNodes.some((l: { source: any; target: any; }) => l.source === link.source.id && l.target === link.target.id);
        }
      }
      ...
      function isConnectedLink(link: any) {
        if (forcedata.connectedPath) {
            ...
            for (let i = 0; i < connectedPaths.length - 1; i++) {
                if (
                    (link.source.id === connectedPaths[i] && link.target.id === connectedPaths[i + 1]) ||
                    (link.source.id === connectedPaths[i + 1] && link.target.id === connectedPaths[i])
                ) {
                    return true;
                }
            }
    ...
    function startAllAnimations() {
      ...
      .delay(1000)
      .duration(2000)
      .attr("r", (d: any) => isHighlighted(d) ? node_radius+5 : node_radius)
      ...
      function startConnectedComp() {
        ...
        forcedata.connectedComponents.forEach((component, componentIndex) => {
              animateConnectedComp(component, nodes, colors[componentIndex % colors.length]);
          });}
      }

      function animateConnectedComp(paths: any[], nodes: any[], targetColor: string | d3.ColorCommonInstance) {
              ...
              setTimeout(() => {
                  ...
                  if (node) {
                      ...
                      if (index < paths.length - 1) {
                          ...
                          svgT.selectAll('line')
                              .filter((d: any) => d.target.id === id || (d.source.id === id && d.target.id === nextId))
                              .transition()
                              .duration(1000)
